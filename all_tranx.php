...
$sales = $class->rawQuery("
    SELECT t.payable_amount,t.amount,t.id,t.client_id,DATE_FORMAT(t.created_at, '%d/%m/%Y') AS created_at,t.status,p.pay_method AS payment_method,
    CONCAT(c.fname,' ',c.lname,' ',c.oname) AS name,t.discount,
    c.phone AS cphone,c.ref AS client FROM transactions AS t LEFT JOIN
                                ...
                                <div class="table-responsive mt-4">
                                    <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                        ...
                                        <tbody>
                                            ...
                                            $remoteRef = $class->simple_encrypt($x->id);
                                            $discount += $x->discount;
                                            $total += $x->amount;
                                            ?>
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    $status = $x->status;
                                                    if($status == 1)
                                                    echo "<button class='btn btn-sm btn-success'><i class='fas fa-check-circle'></i></button>";
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <th colspan='3' class="bg-danger text-white">
                                                    Discount: ₦<?=number_format($discount,0) ?></th>
