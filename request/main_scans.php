...
switch ($_POST['tt']) {
    ...
    case 'sf2A':
        //update the category of test
            try {
                $class = new Functions();
                $description = htmlentities(trim(filter_input(INPUT_POST, 'test_cat')),ENT_QUOTES);
                $id = htmlentities(trim(filter_input(INPUT_POST, 'test_id')),ENT_QUOTES);
                if(!$description){
                    echo json_encode(['status' => 210, 'message' => 'Test Category Name is REQUIRED']);    
                return false;
                }
                  ...
                  $class->updateRecord('id',
                    ['name'],
                    [$description, $id],'test_categories'
                 );
