...
switch ($_POST['tt']) {
    ...
    try {
      ...
      if ($result) {
        ...
        if (password_verify($password, $result->password)) {
          ...
          //store the custom settings in a session
          $config = $class->fetch('settings');
          $json = json_encode($config);
          $_SESSION['site_config'] = $json;
          $url = '';
          switch ($result->role_id) {
            ...
            case '106':
              ...
              break;
            case '107':
              $url .= 'logistics_dashboard.php'; //logistics dashboard
              break;
            case '108':
              $url .= 'labtechnician_dashboard.php'; //labtechnician dashboard
              break;
  ...
  case 't3':
    ...
    try {
      ...
      if (
        !in_array(
          strtolower($productImageExtension),
          $valid_extensions
        )
      ) {
        $uploadOk = 0;
