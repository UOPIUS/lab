...
function makeXHR(e, responseDiv, url, params) {
  ...
  responseDiv.innerHTML =
    '<p class="bg-warning p-2 text-white text-center">Please Wait. . .</p>';
          ...
          switch (params.action) {
            ...
            case "sf1":
              ...
              window.location.href =
                "https://lab.capitalmedicares.com/client_profile.php?refx=" +
                ref_redir;
              break;
            default:
...
function iniNewTest(param) {
  if (
    confirm(
      "You are about to Submit this Form and You cannot Edit After Submission. Are you Sure You Want to Continue?"
    ) === true
  ) {
    //get all selected tests to be performed...
    let div = document
        .getElementById("testsTaken")
        .querySelectorAll(".selected-choices"),
      buildString = "",
      kounter = div.length;
    ...
    makeXHR(param, responseDiv, "request/mk_tranx.php", {
      ...
      token: document
        ...
        ["csrf_token"].getAttribute("content"),
...
function rmvBoxEvt(evt) {
  ...
  const newValue =
    Number(finalTestCost.textContent) - Number(payloadTemp.chosenTestCost);
...
function browseProduct(param) {
  const t = param.value,
    n = new URLSearchParams({
      s: t,
      HTTP_REQUEST_ACTION: "HTTP_FETCH_PRODUCTS",
    }).toString();
  const td = param.parentNode,
    o = td.nextElementSibling.getElementsByTagName("select")[0];

  o.innerHTML = "<option value=''>Choose...</option>";
  let a = new XMLHttpRequest();
  a.open("POST", "../request/xmlHttp.php"),
    a.setRequestHeader("Content-type", "application/x-www-form-urlencoded"),
    a.send(n),
    (a.onload = () => {
      if (200 != a.status) console.log(a.statusText);
      else
        for (
          var e = JSON.parse(a.response), t = e.data.length, n = 0;
          n < t;
          n++
        ) {
          var s = e.data[n],
          ...
          (r.value = s.id),
            r.setAttribute("data-measure", s.measure),
            r.setAttribute("data-unit", s.unit),
            (r.text = s.name),
            o.appendChild(r);
        }
    });
...
function chooseProduct(e) {
  ...
  const td = e.parentElement.nextElementSibling;
  td.getElementsByTagName("input")[0].value =
    e.options[e.selectedIndex].getAttribute("data-unit");
  // `[${e.options[e.selectedIndex].getAttribute("data-measure")}]`;
}
...
function updateProductList(e, b = -1) {
  showOverlay();
  if (e.value == "") return;
  const t = e.value,
    n = new URLSearchParams({
  ...
  const dest = e.getAttribute("data-elementr");
