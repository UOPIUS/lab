...
<head>
    ...
    <title>Inventory Unit Setup | <?= $config->name ?></title>
...
<body class="sb-nav-fixed">
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid">
                    <h4 class="mt-4">Inventory Items</h4>
                    <div class="card mb-4">
                        <div class="card-header">

                            <button class="btn btn-pill btn-danger btn-air-danger float-right"
                                onclick="displayBlock(this)" data-name="Add New Item Category">
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <form action="<?= htmlentities($_SERVER['PHP_SELF']) ?>" id="krd_form" class="mb-4">
                                <div class="row">
                                    <div class="col-lg-6">
                                        <label for="category">Name of Category: <strong
                                                class="text-danger">*</strong></label>
                                        <select name="category_id" id="categoryId" class="form-control">
                                            <option value="">Choose...</option>
                                            <?php $categories = $class->rawQuery("SELECT id, name FROM inventory_categories WHERE status =1");
                                    ...
                                    <div class="col-lg-6">
                                        <label for="category">Unit used to measure product: <strong
                                                class="text-danger">*</strong></label>
                                        <select name="unit_id" id="unitMeasured" class="form-control">
                                            <option value="">Choose...</option>
                                            <?php $unitMeasured = $class->rawQuery("SELECT id, name FROM inventory_units");
                                            foreach ($unitMeasured as $option): ?>
                                            <option value="<?= $option->id ?>"><?= $option->name ?></option>
                                            <?php endforeach; ?>
                                ...
                                <div class="row mt-4">
                                    <div class="col-lg-12">
                                        <label for="category">Name of Product: <strong
                                                class="text-danger">*</strong></label>
                ...
                <div class="modal-body">
                    <form action="<?= htmlentities($_SERVER['PHP_SELF']) ?>" id="editCat" class="mb-4" method="POST">
                        <div class="form-group">
                            <label for="category">Name of unit: <strong class="text-danger">*</strong></label>
                        ...
                        <div class="form-group">
                            <label for="category">Quantity: <strong class="text-danger">*</strong></label>
    ...
    window.onload = () => {
        document.getElementById('krd_form').addEventListener('submit', (e) => {
            ...
            if (!(document.getElementById('categoryId').value && document.getElementById('product')
                    .value && document.getElementById('unitMeasured').value)) {
                responseDiv.innerHTML =
                    "<p class='bg-danger text-center p-2 text-white'>Please Provide a Valid Category Name</p>";
                return false;
            }
            makeXHR(e, responseDiv, '../request/inventory_product.php', {
                categoryId: document.getElementById('categoryId').value,
                product: document.getElementById('product').value,
                unitMeasured: document.getElementById('unitMeasured').value,
    ...
    $("#editModal").on("show.bs.modal", function(event) {
        ...
        var modal = $(this);
        modal.find(".modal-body input[name=product]").val(button.data('name'));
        modal.find(".modal-body input[name=category_id]").val(button.data('category'));
        modal.find(".modal-body input[name=unit_id]").val(button.data('unit'));
        ...
        $("#editCat").submit(function(e) {
            ...
            let url = "../request/inventory_unit.php";
