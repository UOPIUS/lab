...
FROM users_tbl AS u JOIN test_categories c ON c.created_by = u.user_id ORDER BY c.created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Test Category Setup | <?=$config->name ?></title>
...
<body class="sb-nav-fixed">
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid">
                    <h4 class="mt-4">Registered Test Categories</h4>
                    <div class="card mb-4">
                        <div class="card-header">
                            <i class="fas fa-flask mr-1"></i>Categories
                            <button class="btn btn-pill btn-danger btn-air-danger float-right"
                                onclick="displayBlock(this)" data-name="Add New Test Category">
                                <i class="fa fa-plus-circle"></i>&nbsp;Add New Test Category
                            </button>
                        </div>
                ...
                <div class="container-fluid mt-4 d-none" id="divToHide">
                    <div class="card mb-4">
                        <div class="card-header">
                            <h4>Add a new Test Categgory</h4>
                        </div>
                ...
                <div class="modal-body">
                    <form action="<?=htmlentities($_SERVER['PHP_SELF']) ?>" id="editCat" class="mb-4" method="POST">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name='test_cat'>
    ...
    window.onload = () => {
        document.getElementById('krd_form').addEventListener('submit', (e) => {
            ...
            makeXHR(e, responseDiv, 'request/main_scans.php', {
    ...
    $("#editModal").on("show.bs.modal", function(event) {
        ...
        var modal = $(this);
        modal.find(".modal-body input[name=test_cat]").val(button.data('name'));
        modal.find(".modal-body input[name=test_id]").val(button.data('ref'));

        $("#editCat").submit(function(e) {
            $("#edit-response").html(
                '<p class="bg-warning text-white p-2"><i class="fa fa-cog fa-spin"></i>Please Wait. . .</p>'
            );
            let url = "request/main_scans.php";
